...
public:
    ...
    static bool FormatSignature(std::string_view value, std::vector<std::string>& signature)
    {
        for (size_t i = 0; i < value.size(); ++i)
        {
            ...
            else while (i < value.size() && value[i] >= '0' && value[i] <= '9') ++i;
            ...
            if (value.substr(i, 3) == "I64" || value.substr(i, 3) == "I32") i += 3;
            else if (value.substr(i, 2) == "ll" || value.substr(i, 2) == "hh") i += 2;
            else if (i < value.size() && std::string_view("hljztLw").find(value[i]) != std::string_view::npos) ++i;
    ...
    static bool Compatible(std::string_view source, std::string_view translated)
    {
        ...
        return ValidUtf8(source) && ValidUtf8(translated) && !translated.empty() &&
               translated.find("##") == std::string_view::npos &&
               FormatSignature(source, a) && FormatSignature(translated, b) && a == b;
    }
