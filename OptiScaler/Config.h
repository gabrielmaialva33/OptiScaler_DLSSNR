...
class Config
{
  public:
    ...
    // Optional NR after native Ray Reconstruction (DX12).
    ...
    // 0 Auto, 1 Upscaled, 2 Present (swapchain backbuffer). Default 1.
    ...
    // Where the pass runs used to have two keys: HookMethod above, and a DlssNrDx11BridgeHost that
    // gated the D3D11-to-D3D12 bridge. They named one quantity and split it by graphics API, so
    // configuring it correctly meant knowing which D3D version a title uses. HookMethod 2 now means
    // "at present" for both, and with_dx12/dx11_with_dx12_sc.cpp picks the transport. An ini that
    // still carries Dx11BridgeHost is inert rather than wrong.
