...
inline std::optional<float> Float(const char* text, float minimum, float maximum)
{
    ...
    if (text[0] == '+' && value.starts_with('-'))
        return std::nullopt;
    float parsed = 0.0f;
    const auto [end, error] = std::from_chars(value.data(), value.data() + value.size(), parsed);
    if (error != std::errc() || end != value.data() + value.size() || !std::isfinite(parsed) || parsed < minimum ||
        parsed > maximum)
        return std::nullopt;
    return parsed;
}
...
inline std::optional<bool> Bool(const char* text)
{
    if (!text)
        return std::nullopt;
    if (EqualAscii(text, "true"))
        return true;
    if (EqualAscii(text, "false"))
        return false;
    return std::nullopt;
}
...
inline std::optional<uint32_t> Index(std::string_view name)
{
    ...
    if (name.size() <= prefix.size() || !EqualAscii(name.substr(0, prefix.size()), prefix))
        return std::nullopt;
...
inline DlssNrPassSettings Validated(DlssNrPassSettings value)
{
    ...
    {
        if (field && (!std::isfinite(*field) || *field < minimum || *field > 2.0f))
            field.reset();
    };
    ...
    if (value.Preset && *value.Preset > 3)
        value.Preset.reset();
    if (value.Style && *value.Style > 2)
        value.Style.reset();
    return value;
}

inline DlssNrResolvedPassSettings Resolve(const DlssNrResolvedPassSettings& master, const DlssNrPassSettings& sparse,
                                          bool individual)
{
    if (!individual)
        return master;
...
inline void Set(Overrides& overrides, uint32_t index, DlssNrPassSettings value)
{
    ...
    if (value == DlssNrPassSettings {})
        overrides.erase(index);
    else
        overrides.insert_or_assign(index, value);
}
...
template <class Ini> Overrides Load(const Ini& ini)
{
    ...
    for (const auto& section : sections)
    {
        ...
        auto first = [](auto& destination, auto source)
        {
            if (!destination)
                destination = source;
        };
...
template <class Ini> void Save(Ini& ini, const Overrides& overrides)
{
    ...
    for (const auto& section : sections)
        if (Index(section.pItem))
            ownedSections.emplace_back(section.pItem);
    for (const auto& section : ownedSections)
    ...
    for (const auto& [index, raw] : overrides)
    {
        ...
        {
            if (field)
                ini.SetValue(section.c_str(), key, FloatText(*field).c_str());
        };
        ...
        if (value.Style)
            ini.SetValue(section.c_str(), "Style", std::to_string(*value.Style).c_str());
        if (value.Preset)
            ini.SetValue(section.c_str(), "Preset", std::to_string(*value.Preset).c_str());
        if (value.AutoMask)
            ini.SetValue(section.c_str(), "AutoMask", *value.AutoMask ? "true" : "false");
    }
}
} // namespace DlssNr::PassConfig
