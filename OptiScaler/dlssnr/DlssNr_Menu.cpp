    ...
    if (auto ch = ScopedCollapsingHeader(Localization::Label("DLSS Neural Rendering")); ch.IsHeaderOpen())
    {
...

        {
            ...
            HelpMarker("Where the pass runs, and what it runs on.\n\nAuto: the swapchain backbuffer, as soon "
                       ...
                       "runs through OptiScaler's D3D12 bridge (a Vulkan or D3D11 game) there is no "
                       "D3D12 present hook: Auto stays on the upscaler's output and Present runs "
                       "nothing, except on a D3D11 game, where Present hosts the pass on the bridge's "
                       "swapchain and takes effect after a restart.");
        ...
        static const char* nrStyleNames[] = { Localization::Label("Model A (Default)"),
                                              Localization::Label("Model B (Natural)"),
        ...
        HelpMarker("Neural Rendering Model selection (DLSSNR.Style):"
                   ...
                   "\n\nModel B (Natural): subtle contrast (-0.25) and saturation (-0.10) compensation. Preserves "
                   "natural skin tones and softens harsh highlights."
        ...
        {
            ImGui::TextDisabled(
                Localization::Tr("Hook method is set to Present: the swapchain backbuffer is already\n"
        ...
        {
            ImGui::TextDisabled(
                Localization::Tr("The model was trained on finished, sRGB-encoded frames. The upscaler's\n"
...

            {
                ...
                // choice is made on what is available rather than on what sounds best.
                {
                    ...
                    if (source == 1)
                    {
                        ...
                        {
                            const float trim = std::clamp(config->DlssNrWhitePointTrim.value_or_default(), 0.01f, 4.0f);
                ...
                if (wpSource == 2)
                {
                    ...
                    if (!anchors.empty())
                    {
                        ...
                        if (liveScan > 0.0f)
                        {
                            ...
                            ImGui::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f),
                                               Localization::Tr("Scan %.5f  ->  white point %.2f   (%u point%s)"),
                                               liveScan, w, (unsigned) anchors.size(), anchors.size() == 1 ? "" : "s");
                        }
                    }
                    ...
                    if (!anchors.empty())
                    {
                        ...
                        if (ImGui::SliderFloat(Localization::Label("Trim (x the scan)"), &trim, 0.01f, 4.0f, "%.2fx",
                ...
                else
                {
                    ...
                    if (ImGui::SliderFloat(Localization::Label("Paper white"), &wpScale, 0.01f, 2000.0f, "%.2fx",
                                           ImGuiSliderFlags_Logarithmic))
